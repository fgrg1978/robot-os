...
// ---- Page reference counting for COW (AQ9) ----

/// Maximum number of physical pages tracked for COW reference counting.
const MAX_TRACKED_PAGES: usize = 512;

/// Per-page reference count entry: (physical address, refcount).
/// phys == 0 means empty slot.
struct PageRefEntry {
    phys: usize,
    refcount: u16,
}
...
///
/// If the page is not yet tracked, adds it with refcount = 2
...
pub fn page_addref(phys: usize) -> KResult<()> {
    ...
    // Not found — add new entry with refcount 2 (parent + child share).
    for entry in table.iter_mut() {
        if entry.phys == 0 {
            entry.phys = phys;
            entry.refcount = 2;
...
pub fn page_decref(phys: usize) -> bool {
    ...
    // Not tracked — assume single owner, caller should free.
    true
}

/// Get current refcount for a physical page (0 if not tracked).
...
// ---- Copy-on-Write fork (AQ9) ----

/// Create a COW copy of a user page table.
///
/// Marks all user pages as read-only + COW flag in both parent and child.
/// Returns the new page table physical address.
///
/// Only copies L0 (4 KiB) leaf pages with the USER bit set.
/// Kernel mappings (no USER bit) are not copied — use
/// `copy_kernel_entries_to_user()` afterwards.
pub fn fork_cow(parent_pt: usize) -> KResult<usize> {
    let child_pt = create_pagetable()?;
    ...
    for vpn2 in 0..PT_ENTRIES {
        ...
        if !l2_pte.is_valid() { continue; }
        if l2_pte.is_leaf() { continue; } // Skip gigapages (kernel mapping).
        ...
        for vpn1 in 0..PT_ENTRIES {
            ...
            if !l1_pte.is_valid() { continue; }
            if l1_pte.is_leaf() { continue; } // Skip megapages.

            let l0_pt = l1_pte.phys_addr();

            // Walk L0 entries (4 KiB pages).
            for vpn0 in 0..PT_ENTRIES {
                let l0_pte_ptr = (l0_pt + vpn0 * 8) as *mut Pte;
                let l0_pte = unsafe { core::ptr::read_volatile(l0_pte_ptr) };
                if !l0_pte.is_valid() || !l0_pte.is_leaf() { continue; }

                let flags = l0_pte.flags();
                // Only COW user pages.
                if !flags.contains(PteFlags::USER) { continue; }
                ...
                // Reconstruct the virtual address for child mapping.
                let vaddr = (vpn2 << 30) | (vpn1 << 21) | (vpn0 << 12);

                // Map same physical page in child PT with COW flags.
                // Use walk() to allocate intermediate tables as needed.
                let child_pte_ptr = walk(child_pt, vaddr, true)?;
                ...
                let _ = page_addref(phys);
            }
        }
    }

    // Flush parent's TLB — write bits have changed.
    csr::sfence_vma();

    Ok(child_pt)
}

/// Handle a store page fault on a COW page.
///
/// If the faulting page has the COW flag: allocate a new physical page,
/// copy the data, map it as read-write, and clear the COW flag.
/// Returns `Ok(())` if the fault was handled, `Err` if not a COW fault.
pub fn handle_cow_fault(pt: usize, fault_addr: usize) -> KResult<()> {
    let aligned_addr = fault_addr & !(PAGE_SIZE - 1);
    let pte_ptr = walk(pt, aligned_addr, false)?;
    let pte = unsafe { core::ptr::read_volatile(pte_ptr) };

    // Must be valid, have COW flag, and be a leaf.
    ...
    // Restore original flags: add WRITE back, remove COW.
...
///
/// Creates the page table hierarchy but writes a PTE with `valid=0` and
/// `PTE_DEMAND` flag set.  The original mapping flags are stored in the
/// PTE so they can be recovered on fault.
///
/// On first access, `handle_demand_fault()` allocates a physical page,
/// maps it with the stored flags, and zeroes it.
pub fn map_demand(pt_phys: usize, vaddr: usize, flags: PteFlags) -> KResult<()> {
    ...
    // Also reject if already demand-mapped.
    if old.0 & PteFlags::DEMAND.bits() != 0 {
        return Err(KernelError::AlreadyMapped);
    }

    // Write a marker PTE: invalid (no VALID bit) but with DEMAND + original flags.
    // The phys_addr field is unused (set to 0) since no page is allocated yet.
    // We store the desired flags so handle_demand_fault can recover them.
...
///
/// If the faulting PTE has the `DEMAND` flag (and is invalid): allocate
/// a physical page, zero it, and map it with the original flags.
/// Returns `Ok(())` if handled, `Err` if not a demand page fault.
pub fn handle_demand_fault(pt: usize, fault_addr: usize) -> KResult<()> {
    let aligned_addr = fault_addr & !(PAGE_SIZE - 1);
    let pte_ptr = walk(pt, aligned_addr, false)?;
    let pte = unsafe { core::ptr::read_volatile(pte_ptr) };

    // Must be invalid but have the DEMAND flag.
    if pte.is_valid() {
        return Err(KernelError::AlreadyMapped);
    }
    ...
    // Recover the original flags (everything except DEMAND).
    let stored_flags = PteFlags::from_bits_truncate(pte.0 & !PteFlags::DEMAND.bits());

    // Allocate a physical page (alloc_page returns zeroed pages).
    let new_page = pmm::alloc_page()?;
    let new_phys = new_page.as_usize();

    // Zero the page explicitly for safety (defense in depth).
    unsafe {
        core::ptr::write_bytes(new_phys as *mut u8, 0, PAGE_SIZE);
    }

    // Map with the original flags + VALID, without DEMAND.
    let final_flags = stored_flags | PteFlags::VALID;
