...
pub use pure::{
    ...
    OTA_MAX_IMAGE_SIZE, OTA_PLATFORM_ESP32C3, OTA_PLATFORM_K1,
    OTA_PLATFORM_QEMU, OTA_PLATFORM_VF2, SLOT_A, SLOT_B, SLOT_R,
...
pub use secure_boot::{
    BootTrust, SecureBootInfo,
    secure_boot_verify_slot, secure_boot_require_signature,
    secure_boot_set_require_signature, secure_boot_info,
    secure_boot_sig_path, SECURE_BOOT_PUBKEY,
...
///
/// Our serialized records are ~180 bytes; 512 keeps generous headroom
/// and matches the FAT32 sector size.
...
pub fn ota_read_boot_meta() -> BootMeta {
    ...
    if fd >= 0 {
        ...
        robot_os_fs::vfs_close(&mut fd_table, fd);
        if n > 0 {
            return parse_boot_meta(&buf[..n as usize]);
        }
    }
