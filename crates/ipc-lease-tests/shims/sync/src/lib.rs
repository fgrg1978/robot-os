...
//!
//! **WHY this exists.** `crates/ipc/src/{lease,rpc,port}.rs` all sit on
//! `robot_os_sync::SpinLock`, and the real `SpinLock` calls
//! `robot_os_arch::csr` (RV64 `sstatus` reads/writes in inline asm) from
//! `lock_irqsave()`. That cannot be compiled or executed on the host, so the
//! `#[path]` trick used by `crates/cap-tests` stops at the first
//! `lock_irqsave()`. This crate provides the same *surface* backed by a
